package com.dspl.malkey.components
{
	import flash.events.Event;
	import flash.events.FocusEvent;
	import flash.events.KeyboardEvent;
	
	import mx.events.FlexEvent;
	
	import spark.components.TextInput;
	
	public class CaptionTextInput extends spark.components.TextInput
	{
		public var _showsCaption:Boolean;
		public var _caption:String;
		
		public function CaptionTextInput()
		{
			super();
			this.addEventListener(FocusEvent.FOCUS_IN, onFocusIn);
			this.addEventListener(FocusEvent.FOCUS_OUT, onFocusOut);
			this.addEventListener(FlexEvent.CREATION_COMPLETE,fieldCreationComplete,false,0,false);
		}
		
		[Bindable("change")]
		[Bindable("textChanged")]
		override public function set text(value:String):void {
			
			this._showsCaption = false;
			super.text = value;
		}
		
		override public function get text():String {
			if (this._showsCaption) {
				return "";
			}
			else return super.text;
			
		}
		
		public function refreshCaption():void
		{
			dispatchFocusOutEvent();
		}
		
		private function fieldCreationComplete(event:FlexEvent):void
		{
			dispatchFocusOutEvent();
		}
		
		private function dispatchFocusOutEvent():void
		{
			onFocusOut(new FocusEvent(FocusEvent.FOCUS_OUT,false,false));
		}
		
		/**
		 * The value to be shown if the textfield has no text (this.text == '')
		 */
		public function set caption(value:String):void {
			if (super.text == "") {
				super.text = value;
				this._caption = value;
				this._showsCaption = true;
			}
		}
		
		private function onFocusIn(ev:FocusEvent):void {
			if (this._showsCaption) {
				this._showsCaption = false;
				super.text = "";
				this.setStyle("fontStyle","normal");
				this.setStyle("color","#000000"); 
			}
		}
		
		private function onFocusOut(ev:FocusEvent):void {
			if (this.text == "") {
				this._showsCaption = true;
				super.text = _caption;
				this.setStyle("fontStyle","italic");
				this.setStyle("color","#9D9D9D"); 
			}
		}
	}
}