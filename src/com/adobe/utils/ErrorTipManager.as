package com.adobe.utils
{
	...
	public class ErrorTipManager
	{
				...
				if (targets && (targets.length > 0)) {
					...
					var id:uint = setTimeout(function():void {
						...
						for each (var target:DisplayObject in targets) {
							...
							try{
							var tool:ToolTip=getErrorTip(target);
							Alert.show("taget: " + target.name + "   toolTipVisible: " + tool.visible.toString());
							}catch(error:Error){
								Alert.show("testError: " + error.message);
							}
	     ...
	     */
	    private static function positionErrorTip(errorTip:IToolTip, target:DisplayObject, bringInFront:Boolean = false):void {
	    	if (!errorTip || !target) {
	    		return;
	    	}
	        var x:Number;
		...
		public static function showErrorTip(target:Object, error:String = null):void {
			var errorTip:IToolTip = createErrorTip(target, error);
			if (errorTip) {
				if(errorTip.text.toString().length>0)
					errorTip.visible = true;
			}
		}
		
		
		public static function showACErrorTip(target:Object, error:String = null):void {
			var errorTip:IToolTip=getErrorTip(target);
			if(errorTip==null)
				errorTip=createErrorTip(target, error);
			if (errorTip) {
				errorTip.visible = true;
			}
		}
		...
		public static function removeAllErrorTips():void{
